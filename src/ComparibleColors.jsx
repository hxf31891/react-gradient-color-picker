  ...
  return(
    <div style={{height: openComparibles ? 216 : 0, width: '100%', transition: 'all 120ms linear'}}>
      <div style={{paddingTop: 11, display: openComparibles ? '' : 'none', ...psRl }}>
        ...
        <div style={{borderRadius: 5, overflow: 'hidden', ...df, ...jfe }}>{triad?.map((c, key) => (<div key={key} style={{width: 'calc(100% / 3)', height: 28, background: c.toHexString()}} onClick={() => handleClick(c)} />))}</div>
        <div style={{textAlign: 'center', color: '#323136', fontSize: 12, fontWeight: 500, marginTop: 3}}>Tetrad</div>
        <div style={{borderRadius: 5, overflow: 'hidden', ...df, ...jfe }}>{tetrad?.map((c, key) => (<div key={key} style={{width: '25%', height: 28, background: c.toHexString()}} onClick={() => handleClick(c)} />))}</div>
      </div>
    </div>
  )
}
