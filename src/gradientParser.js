...
export const gradientParser = (input = '') => {
  ...
  function getAST() {
    ...
    const ast0 = ast[0]
    const checkSelected = ast0?.colorStops?.filter(c => isUpperCase(c.value)).length;
    ...
    const getGradientObj = () => {
      if (checkSelected > 0) {
        ...
        return {...ast0, colorStops: ast0.colorStops.map((c, i) => ({...c, value: val(c, i)}))}
      }
    }
  ...
  const convertHsl = (val) => {
    ...
    let { r, g, b, a } = tinycolor({h: hsl[0], s: hsl[1], l: hsl[2], a: hsl[3] || 1}).toRgb()
    return  {
  ...
  const convertHsv = (val) => {
    ...
    let { r, g, b, a } = tinycolor({h: hsv[0], s: hsv[1], v: hsv[2], a: hsv[3] || 1}).toRgb()
    return  {
      value: `${capIt ? 'RGBA' : 'rgba'}(${r}, ${g}, ${b}, ${a})`
    }
  }
